...
class MatplotlibPlotter(BasePlotter):
    ...
    def __init__(self, theme: PlotTheme = PlotTheme.DEFAULT, figsize: Tuple[int, int] = (12, 6)):
        try:
            import matplotlib.pyplot as plt
            ...
            raise ImportError("matplotlib is required for this backend. Install with: pip install matplotlib")
    ...
    def plot_portfolio_value(
        ...
        if 'total_value' in history.columns:
            ax.plot(history.index, history['total_value'],
                    linewidth=2, color='#2E86AB', label='Total Value')

        if 'cash' in history.columns:
            ax.plot(history.index, history['cash'],
                    linewidth=1.5, color='#A23B72', alpha=0.7, label='Cash')

        if 'positions_value' in history.columns:
            ax.plot(history.index, history['positions_value'],
                    linewidth=1.5, color='#F18F01', alpha=0.7, label='Positions')
    ...
    def plot_portfolio_composition(
        ...
        if len(filtered_weights) < len(weights):
            other_weight = sum(v for k, v in weights.items() if k not in filtered_weights)
            if other_weight > 0:
    ...
    def plot_returns(
        ...
        # Histogram
        axes[0].hist(returns.dropna(), bins=50, alpha=0.7, color='#2E86AB', edgecolor='black')
        axes[0].axvline(returns.mean(), color='red', linestyle='--', linewidth=2, label=f'Mean: {returns.mean():.2%}')
        ...
        # Time series
        axes[1].plot(returns.index, returns.values, linewidth=1, alpha=0.7, color='#2E86AB')
    ...
    def plot_drawdown(
        ...
        fig, axes = self.plt.subplots(2, 1, figsize=kwargs.get('figsize', self.figsize), sharex=True)

        # Equity curve
        axes[0].plot(equity_curve.index, equity_curve.values, linewidth=2, color='#2E86AB')
        axes[0].set_ylabel('Portfolio Value ($)', fontsize=10)
        axes[0].set_title('Equity Curve', fontsize=11, fontweight='bold')
        axes[0].grid(True, alpha=0.3)

        # Drawdown
        axes[1].fill_between(drawdown.index, drawdown.values, 0, alpha=0.5, color='#A23B72')
        axes[1].set_xlabel('Date', fontsize=10)
...
class PlotlyPlotter(BasePlotter):
    ...
    def __init__(self, theme: PlotTheme = PlotTheme.DEFAULT):
        try:
            ...
            raise ImportError("plotly is required for this backend. Install with: pip install plotly")
    ...
    def plot_price_history(
        ...
        specs = [[{"secondary_y": False}], [{"secondary_y": False}]] if has_volume else [[{"secondary_y": False}]]
    ...
    def plot_returns(
        ...
        fig = self.make_subplots(
            rows=1, cols=2,
            subplot_titles=('Returns Histogram', 'Returns Over Time')
        )
...
class Plotter(BaseModel):
    ...
    """
    backend: PlotBackend = Field(default=PlotBackend.PLOTLY, description="Plotting backend")
    ...
    def plot_correlation_matrix(
        ...
        if self.backend == PlotBackend.PLOTLY:
            fig = self._plotter.go.Figure(data=self._plotter.go.Heatmap(
                z=corr.values,
                x=corr.columns,
                y=corr.columns,
                colorscale='RdBu',
    ...
    def plot_performance_summary(
        ...
        if self.backend == PlotBackend.PLOTLY:
            fig = self._plotter.go.Figure(data=[
    ...
    def plot_universe_analysis(
        ...
        if self.backend == PlotBackend.PLOTLY:
            fig = self._plotter.go.Figure(data=[
                self._plotter.go.Bar(x=labels, y=values, marker_color='#F18F01')
            ])
            fig.update_layout(
                title=title,
                xaxis_title="Ticker",
